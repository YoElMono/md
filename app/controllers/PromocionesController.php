...
class PromocionesController extends ControllerBase {
	...
	public function initialize() {
		...
		$this->setJS("promociones");
		$this->Security = new Security();
		$this->Modulo = "sub_promociones_promociones"; 
		$this->Controller = "promociones";
	}
	...
	public function getAction(){
		...
		$Result = Promociones::find(array(
			"columns" => "id,nombre,descripcion,fecha_caducidad,status",
		    "conditions" => "status!=2 and id_negocio=".$_SESSION['id_negocio'],
		   //"limit" => 4
		));	
		//1 activo
		//0 inactivo
		...
		if( count($Result) > 0 ){
			foreach($Result as $key => $value){
				...
				if( (int) $_SESSION["PermisosUser"][$this->Modulo . "_edit"] == 1 ){
					$Buttons .= '<a href="promociones/edit/'.trim($value->id).'/" class="btn btn-sm btn-icon btn-primary"><i class="fa fa-pencil"></i></a>';
				}
				...
				$img="<img src='../tmp/promociones/".$value->img."' width=50 height=50 >";

				$Data["aaData"][] = array(
					"id" => trim($value->id),
					"nombre" => utf8_decode(trim($value->nombre)),
					"descripcion" => utf8_decode(trim($value->descripcion)),
					"fecha_caducidad" => trim($value->fecha_caducidad),
	...
	public function indexAction(){
		...
		$this->setHeaderMenu("Promociones" , "Listado de promociones" , $this->Controller , "");		
	...
	public function newAction(){
		...
		$Folder =  __DIR__  . "/../../../tmp/promociones/";
		@mkdir($Folder , 777);
		...
		if($this->request->isPost()){
			...
			$_POST["descripcion"] = utf8_encode($_POST["descripcion"]);
			...
			if( count($Result) <= 0 ){
				...
				if($Table->save()){
					...
					if($_FILES['img']["name"] != ""){
						...
						$ruta=$Folder.$name;
						$directorio='promociones';
						$this->Miniaturas($ruta,120,$name,$directorio);			
		...
		$this->view->img = '';
		$this->ajaxBody($this->Title);
		$this->setHeaderMenu("Promociones" , "Nueva Promociones" , $this->Controller , "Registro");		
		$this->view->textarea = "";
...
public function editAction($id=""){
		...
		if($this->request->isPost()){
			...
			$Tabla = Promociones::findFirst(array(
				"columns" => "*",
			    "conditions" => "id=:id:",
		...
		$this->ajaxBody($this->Title);
		$this->setHeaderMenu("Clasificaciónes" , " " , $this->Controller , "Editar");		
		$this->view->formAction = $this->Controller."/edit/" .$id;
		//$this->view->Departamentos = $this->getDepartamentos();
		$DataForm = array("data" => array());
		$Tabla = new Promociones();
		...
		foreach($Result as $value){
			$DataForm["data"] = array(
				...
				"contenido" => utf8_decode($value->contenido),
				"fecha_caducidad" => utf8_decode($value->fecha_caducidad),
